...
八、高级视觉任务算子
1. 目标检测
cv::CascadeClassifier::detectMultiScale()： Haar 级联检测（基于训练好的模型，检测人脸、眼睛、车牌等，如 haarcascade_frontalface_default.xml）；
...
cv::filter2D()：自定义卷积滤波（手动设计卷积核，实现自定义效果，如锐化、边缘检测）。"# mediakit" 
